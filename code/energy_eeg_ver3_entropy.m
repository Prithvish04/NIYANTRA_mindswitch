...
Entropy_alpha_subjects=[];
Entropy_beta_subjects=[];
Entropy_theta_subjects=[];
...
for i=3:(size_file(2))    
    ...
    for j=1:size_sheet(2)
        ...
        % calculating energy for individual bands
            Entropy_alpha=entropy(alpha_filt_eeg);
            Entropy_beta=entropy(beta_filt_eeg);
            Entropy_theta=entropy(theta_filt_eeg);
        
        Entropy_vector_alpha(j)=Entropy_alpha;
        Entropy_vector_beta(j)=Entropy_beta;
        Entropy_vector_theta(j)=Entropy_theta;
    end
    cd ..
    Entropy_alpha_subjects=[Entropy_alpha_subjects;Entropy_vector_alpha]
    Entropy_beta_subjects=[Entropy_beta_subjects;Entropy_vector_beta]
    Entropy_theta_subjects=[Entropy_theta_subjects;Entropy_vector_theta]
end
